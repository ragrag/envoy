...
- `stdin`: The standard input to the program (optional).
...
- `time` (optional): The actual time (in seconds) the program took to execute, in case of `COMPILATION_ERROR` this is not provided
- `memory` (optional): The actual memory (in KB) the program used, in case of `COMPILATION_ERROR` this is not provided
